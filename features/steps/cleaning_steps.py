...
def step__check_that_cleaned_and_original_lines_are_equal(context):
    assert_that(context.cleaned_lines.splitlines(), 
                equal_to(context.original_lines.splitlines()))


@then('the text consists of the following lines')
def step__check_that_cleaned_lines_are_as_given(context):
    assert_that(context.cleaned_lines.splitlines(), 
                equal_to(context.text.splitlines()))
   


@given('a text file named "{filename}"')
def step__given_a_text_file(context, filename):
    source_name = os.path.join(cfg.testDirectories["rawTxtDir"], filename)
...
def step__given_an_test_oracle(context):
    ...
    oracle_name = os.path.join(cfg.testDirectories["cleanTxtDir"], filename)
...
def step__create_a_cleaned_file(context):
    filename = derived_filename(context.filename, 'cln_', '.txt')
    target_name = os.path.join(cfg.testDirectories["cleanTxtDir"], filename)
